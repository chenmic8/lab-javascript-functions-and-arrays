...
function sum(numbers) {
  ...
  numbers.forEach((number) => {
    if (typeof number === "string") {
      sum += number.length;
    } else if (typeof number != "boolean" && typeof number != "number") {
...
// Bonus - Iteration #4.1
function avg(words) {
  return averageWordLength(words);
}
...
function greatestProduct(twoDimensionalArr) {
  let max = 0,
    result;
  twoDimensionalArr.forEach((oneDimensionalArr, i) => {
    oneDimensionalArr.forEach((number, j) => {
